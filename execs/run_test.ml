...
let test_parse_int_negative () =
  check exp "same int" (parse_exp (`Atom "-5")) (Num (Int64.neg 5L))
...
let test_parse_compound_2 () =
  check exp "same expr"
    (parse_exp (`List [`Atom "or" ; `List [`Atom "and" ; `Atom "true"; `Atom "x"]; `Atom "false"]))
    (Prim2 (Or, Prim2 (And, Bool true, Id "x"), Bool false))

let test_parse_compound_3 () =
  check exp "same expr"
    (parse_exp (`List [
      `Atom "not" ; `List [
        `Atom "xor" ; 
        `List [`Atom "<" ; 
          `Atom "3"; 
          `List [`Atom "add1" ; `List [`Atom "+" ; `Atom "x" ; `Atom "5"]]]; 
        `List [ `Atom ">=" ; 
          `List [`Atom "-" ; 
            `Atom "7" ; 
            `List [`Atom "sub1" ; `Atom "4"]] ; 
          `List [`Atom "*" ; `Atom "y" ; `Atom "10"]]]]))
    (Prim1 (Not, 
      Prim2 (Xor, 
        Prim2 (Lt, Num 3L, Prim1 (Add1, Prim2 (Add, Id "x", Num 5L))), 
        Prim2 (Gte, Prim2 (Sub, Num 7L, Prim1 (Sub1, Num 4L)), Prim2 (Mul, Id "y", Num 10L)))))

let test_parse_compound_4 () =
  check exp "same expr"
    (parse_exp (`List [
      `Atom "not" ; `List [
        `Atom "not" ; `List [
          `Atom "not" ; `List [
            `Atom "and" ; 
            `List [ 
              `Atom "or" ; `List [`Atom "==" ; `Atom "3"; `Atom "3"] ; `List [`Atom "!=" ; `Atom "3"; `Atom "3"]] ;
              `List [ `Atom "not" ; `List [
                `Atom "and" ; `List [`Atom ">" ; `Atom "3" ; `Atom "3"] ; `List [`Atom "<=" ; `Atom "3" ; `Atom "3"]]]]]]]))
    (Prim1 (
      Not, Prim1 (
        Not, Prim1 (
          Not, Prim2 (
            And, 
            Prim2 (Or, Prim2 (Eq, Num 3L, Num 3L), Prim2 (Neq, Num 3L, Num 3L)),
            Prim1 (Not, Prim2 (And, Prim2 (Gt, Num 3L, Num 3L), Prim2 (Lte, Num 3L, Num 3L))))))))
...
let test_parse_let () =
  check exp "same expr"
    (parse_exp (`List [
      `Atom "let" ; 
      `List [`Atom "x" ; `List [`Atom "+" ; `Atom "8" ; `Atom "3"]] ; 
      `List [`Atom "+" ; `Atom "3"; `Atom "x"]]))
    (Let ("x", Prim2 (Add, Num 8L, Num 3L), Prim2 (Add, Num 3L, Id "x")))
...
let test_parse_let_nested () =
  check exp "same expr"
    (parse_exp (`List [
      ...
      `List [`Atom "x" ; `List [`Atom "+" ; `Atom "8" ; `Atom "3"]] ; 
      `List [`Atom "let" ; 
        `List [`Atom "y" ; `List [`Atom "sub1" ; `Atom "x"]]; 
        `List [`Atom "let" ;
          `List [`Atom "x" ; `Atom "1"] ;
          `List [`Atom "*" ; `Atom "x" ; `Atom "y"]]]]))
    (Let ("x", Prim2 (Add, Num 8L, Num 3L),
      Let ("y", Prim1 (Sub1, Id "x"),
        Let ("x", Num 1L,
          Prim2 (Mul, Id "x", Id "y")))))
  
let test_parse_let_fail_1 () =
  let sexp = `List [`Atom "let" ; 
    `List [`Atom "1" ; `Atom "2"] ; 
    `List [`Atom "+" ; `Atom "3"; `Atom "1"]] in
  check_raises "Should raise failwith"
    (Failure "variable name can't be a number")
    (fun () -> ignore @@ parse_exp sexp)

let test_parse_let_fail_2 () =
  let sexp = `List [`Atom "let" ; 
    `List [`Atom "+" ; `Atom "2"] ; 
    `List [`Atom "+" ; `Atom "3"; `Atom "+"]] in
  check_raises "Should raise failwith"
    (Failure "cannot override '+'")
    (fun () -> ignore @@ parse_exp sexp)
...
let test_parse_if () =
  check exp "same expr"
    (parse_exp (`List [
      `Atom "if" ; `List [`Atom "or" ; `Atom "true"; `Atom "false"]; 
        `List [`Atom "+" ; `Atom "8" ; `Atom "3"] ; 
        `Atom "2"]))
    (If (Prim2 (Or, Bool true, Bool false), Prim2 (Add, Num 8L, Num 3L), Num 2L))

let test_parse_if_nested () =
  check exp "same expr"
    (parse_exp (`List [
      `Atom "if" ; `List [`Atom "or" ; `Atom "true"; `Atom "false"]; 
        `List [`Atom "if" ; `List [`Atom "<" ; `Atom "1" ; `Atom "3"] ; `Atom "8" ; `Atom "3"] ; 
        `List [`Atom "if" ; `List [`Atom "!=" ; `Atom "4" ; `Atom "4"] ; `Atom "4" ; `Atom "5"]]))
    (If (Prim2 (Or, Bool true, Bool false), 
      If (Prim2 (Lt, Num 1L, Num 3L), Num 8L, Num 3L),
      If (Prim2 (Neq, Num 4L, Num 4L), Num 4L, Num 5L)))

let test_parse_compound_5 () =
  check exp "same expr"
  (parse_exp (`List [
    `Atom "if" ; `List [`Atom "or" ; `Atom "true"; `Atom "false"]; 
      `List [`Atom "let" ; `List [`Atom "x"; `Atom "2"] ; `List [`Atom "if" ; `List [`Atom "<" ; `Atom "x" ; `Atom "3"] ; `Atom "-8" ; `Atom "3"]] ; 
      `List [`Atom "if" ; `List [`Atom "!=" ; `Atom "4" ; `Atom "4"] ; `Atom "4" ; `List [`Atom "let" ; `List [`Atom "x" ; `Atom "4"] ; `Atom "x"]]]))
  (If (Prim2 (Or, Bool true, Bool false), 
    Let ("x", Num 2L, If (Prim2 (Lt, Id "x", Num 3L), Num (Int64.neg 8L), Num 3L)),
    If (Prim2 (Neq, Num 4L, Num 4L), Num 4L, Let ("x", Num 4L, Id "x"))))
...
let test_interp_num_n () =
  check value "same int" (interp (Num (Int64.neg 42L)) empty_env empty_fenv) (NumV (Int64.neg 42L))
...
let ocaml_tests = [
  "parse", [
    ...
    test_case "A negative number" `Quick test_parse_int_negative ;
  ...
  "interp", [
    ...
    test_case "A negative number" `Quick test_interp_num_n ;
